...
export default function Expenses() {
  ...
  function defaultForm() {
    ...
    return { amount: '', date: today, vendor: '', category: 'Others', notes: '' };
  }
    ...
    <div className="space-y-8" data-testid="expenses-page">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-charcoal-muted mt-2 max-w-2xl">
            Filter by month, year or category. Medical & Business are treated as deductible for the SARS calculation.
          </p>
        </div>
        <button
          onClick={openAdd}
      ...
      <div className="border border-line bg-surface p-4 flex flex-wrap gap-3" data-testid="expense-filters">
        ...
        <Select
          ...
          options={[{ value: '', label: 'All' }, ...MONTH_NAMES.map((m, i) => ({ value: i + 1, label: m }))]}
          testid="filter-month"
        />
        <Select
          label="Category"
      ...
      <div className="border border-line bg-surface" data-testid="expense-list">
        ...
        {list.length === 0 ? (
          ...
          list.map((x) => (
            <div
              key={x.id}
              className="grid grid-cols-6 border-b border-line/70 hover:bg-surfaceAlt/40 transition-colors items-center"
            ...
            >
              ...
              <Td>
                <span className={`text-xs font-semibold px-2 py-0.5 ${x.source === 'OCR' ? 'bg-moss/10 text-moss border border-moss/20' : 'bg-surfaceAlt text-charcoal-muted border border-line'}`}>
                  {x.source}
                </span>
              </Td>
              <Td align="right" className="flex items-center justify-end gap-3">
                ...
                <button onClick={() => openEdit(x)} data-testid={`expense-edit-${x.id}`} className="text-charcoal-muted hover:text-moss">
                  <Pencil size={14} />
                </button>
                <button onClick={() => remove(x.id)} data-testid={`expense-delete-${x.id}`} className="text-terra hover:text-terra-hover">
                  <Trash2 size={14} />
                </button>
              </Td>
            </div>
      ...
      {(adding || editing) && (
        <div className="fixed inset-0 bg-charcoal/40 flex items-center justify-center p-4 z-50" data-testid="expense-modal">
          <form onSubmit={save} className="bg-surface w-full max-w-md border border-line p-6 space-y-4">
            ...
            {error && (
              <div className="border border-terra/40 bg-terra/10 text-terra text-sm px-3 py-2">{error}</div>
            )}
            <div className="flex justify-end gap-2 pt-2">
              <button type="button" onClick={close} className="border border-line px-4 py-2 text-sm hover:bg-surfaceAlt">
                Cancel
              </button>
              <button type="submit" data-testid="expense-save-btn" className="bg-moss text-page px-4 py-2 text-sm font-semibold hover:bg-moss-hover">
                Save
              </button>
            </div>
          </form>
